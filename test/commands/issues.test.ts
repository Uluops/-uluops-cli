...
describe('issues get', () => {
  ...
  it('should fetch full details with --full', async () => {
    ...
    mockClient.issues.getDetails.mockResolvedValue({
      ...
      notes: [
        { noteType: 'context', content: 'This is a note about the issue' },
      ],
      history: [
        {
...
describe('issues update', () => {
  it('should update issue status', async () => {
    mockClient.issues.updateStatus.mockResolvedValue({
      ...createIssue(),
      status: 'completed',
    });
    const output = captureOutput();
...
describe('issues add-note', () => {
  it('should add note to issue', async () => {
    mockClient.issues.addNote.mockResolvedValue({
      id: 'note-1',
      content: 'hello',
    });
    const output = captureOutput();
...
describe('issues undo', () => {
  it('should undo last status change', async () => {
    mockClient.issues.undoLastChange.mockResolvedValue({
      ...createIssue(),
      status: 'open',
    });
    const output = captureOutput();
...
describe('issues create', () => {
  it('should create a user-submitted issue', async () => {
    mockClient.issues.create.mockResolvedValue(
      createIssue({ title: 'Manual bug report' }),
    );
    const output = captureOutput();
    await parse(
      'issues',
...
describe('issues edit', () => {
  it('should edit issue metadata', async () => {
    mockClient.issues.update.mockResolvedValue(
      createIssue({ title: 'Updated title', severity: 'high' }),
    );
    const output = captureOutput();
    await parse(
      'issues',
    ...
    expect(mockClient.issues.update).toHaveBeenCalledWith(
      'abc-123',
      expect.objectContaining({
        title: 'Updated title',
        severity: 'high',
...
describe('issues restore', () => {
  it('should restore a soft-deleted issue', async () => {
    mockClient.issues.restore.mockResolvedValue({
      ...createIssue(),
      status: 'open',
    });
    const output = captureOutput();
...
describe('issues by-fingerprint', () => {
  it('should fetch issue by fingerprint', async () => {
    mockClient.issues.getByFingerprint.mockResolvedValue(
      createIssue({ title: 'Fingerprint match' }),
    );
    const output = captureOutput();
    await parse(
      'issues',
...
describe('issues update-by-fingerprint', () => {
  it('should update issue status by fingerprint', async () => {
    ...
    expect(mockClient.issues.updateStatusByFingerprint).toHaveBeenCalledWith(
      'abc123hash',
      'my-proj',
      {
        status: 'completed',
...
describe('error handling', () => {
  it('should delegate to handleOpsError on list failure', async () => {
    ...
    await expect(parse('issues', 'list', 'my-proj')).rejects.toThrow(
      'API fail',
    );
  ...
  it('should delegate to handleOpsError on update failure', async () => {
    ...
    await expect(
      parse('issues', 'update', 'abc-123', '--status', 'completed'),
