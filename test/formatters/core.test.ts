...
function createMetrics(
  overrides: Partial<AgentResult['metrics']> = {},
): AgentResult['metrics'] {
  return {
...
function createValidatorResult(
  overrides: Partial<ValidatorAgentResult> = {},
): ValidatorAgentResult {
  return {
    type: 'agent',
    name: 'code-validator',
...
function createGenericAgentResult(
...
describe('formatAgentResult', () => {
  ...
  it('renders score and threshold for validator agents', () => {
    const result = formatAgentResult(
      createValidatorResult({ score: 92, maxScore: 100, threshold: 80 }),
    );
  ...
  it('renders duration in seconds', () => {
    const result = formatAgentResult(
      createValidatorResult({ durationMs: 5000 }),
    );
  ...
  it('renders duration in milliseconds for short runs', () => {
    const result = formatAgentResult(
      createValidatorResult({ durationMs: 500 }),
    );
  ...
  it('renders duration in minutes for long runs', () => {
    const result = formatAgentResult(
      createValidatorResult({ durationMs: 125000 }),
    );
  ...
  it('renders dashboard URL when present', () => {
    const result = formatAgentResult(
      createValidatorResult({
        dashboardUrl: 'https://dash.uluops.com/run/123',
      }),
    );
  ...
  it('omits dashboard URL when not present', () => {
    const result = formatAgentResult(
      createValidatorResult({ dashboardUrl: undefined }),
    );
  ...
  it('renders a trackingError notice with an upgrade URL for PROJECT_LIMIT', () => {
    const result = formatAgentResult(
      createValidatorResult({
        ...
        trackingError: {
          ...
          details: {
            upgradeUrl:
              'https://registry.uluops.ai/orgs/acme/settings/billing?source=api',
          },
        },
      }),
    );
    ...
    expect(result).toContain(
      'upgrade: https://registry.uluops.ai/orgs/acme/settings/billing?source=api',
    );
    expect(result).not.toContain('Dashboard:');
  });
  ...
  it('omits the upgrade URL for a non-cap trackingError', () => {
    const result = formatAgentResult(
      createValidatorResult({
        trackingError: { statusCode: 500, message: 'server error' },
      }),
    );
  ...
  it('shows a completeness badge when the run is not complete', () => {
    const result = formatAgentResult(
      createGenericAgentResult({ decision: 'PASS', completeness: 'partial' }),
    );
  ...
  it('omits the completeness badge for a complete run', () => {
    const result = formatAgentResult(
      createGenericAgentResult({ decision: 'PASS', completeness: 'complete' }),
    );
  ...
  it('omits the completeness badge when completeness is absent', () => {
    const result = formatAgentResult(
      createGenericAgentResult({ decision: 'PASS' }),
    );
    expect(result).not.toContain('Completeness:');
  });
  ...
  it('lists degradation markers only under verbose', () => {
    ...
    expect(verbose).toContain(
      '[DEGRADED] budget.forced-wrap-up — coverage may be partial',
    );
  });
});
...
describe('formatExecutionResult', () => {
  ...
  it('renders workflow type correctly', () => {
    const result = formatExecutionResult(
      createExecResult({ type: 'workflow', name: 'ship' }),
    );
  ...
  it('omits score when undefined', () => {
    const result = formatExecutionResult(
      createExecResult({ score: undefined }),
    );
    expect(result).not.toContain('Score:');
  });
  ...
  it('renders dashboard URL when present', () => {
    const result = formatExecutionResult(
      createExecResult({ dashboardUrl: 'https://example.com' }),
    );
