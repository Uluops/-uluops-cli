          ...
          if (!id) {
            ...
            const result = await withSpinner(
              ctx,
              {
                ...
                failure: 'Failed to fetch languages',
              },
            ...
            if (ctx.json) {
              ...
              for (const lang of result.languages) {
                console.log(
                  `  ${lang.abbreviation.padEnd(5)} v${lang.currentVersion.padEnd(8)} ${lang.displayName}`,
                );
              }
            ...
            if (options.output) {
              await writeFile(
                options.output,
                JSON.stringify(language.schema.content, null, 2),
                'utf-8',
              );
              console.log(
